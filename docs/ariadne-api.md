...
Without a source field, `claim` itself becomes submitted source text. It has no publication date and no inferred outbound links. A fetched provider proposal is exposed as a `candidate` edge from the real candidate source to the submitted query anchor and becomes a traversal root. This discovery relationship is not a validated provenance edge and does not manufacture chronology; all subsequent real document-to-document relationships use the existing directional validation rules. Submitted nodes have `source_kind: "submitted"`; their `url` is an internal `https://submitted.ariadne.invalid/...` identity needed by the existing document contract, not a fetched/publication URL. Do not offer it as an external source link. `root.url` is `null` for this case. Prefer a real `seed_url` when publication chronology is part of the demonstration.

`max_depth` defaults to 5, accepts 0–10, and counts validated upstream hops. Claim-only candidate roots are bootstrap discovery and do not consume a validated hop. `max_provider_requests` defaults to 10 and accepts 1–10 per invocation. A limit can stop or pause a bounded run; `complete` does not claim the graph contains every possible ancestor. `include_ai_evidence` remains accepted for request compatibility but is deprecated: `true` produces an explicit warning and no AI-writing check. This endpoint runs bibliography proposals.

Execution is selected by server configuration, never by a request field. `USE_MOCKS=true` uses the same outbound-link stand-in and offline acquisition as `demo:cohen -- --mock`. Otherwise `createBibliographyProposer` uses live GPTZero with its existing cached-demo fallback policy, and acquisition uses `DirectHttpFetcher`. No Browserbase or search/discovery pipeline is invoked.
...
`AriadneNode` reuses `TreeNode` (canonical identity, URL/mirrors, publisher/title, timestamps and their provenance, passage, citations, seed flag) and adds `source_kind`. Authors are not extracted by the current document model, so they are not invented. `root` is the submitted seed, not necessarily the earliest upstream document. A failed seed acquisition has an unavailable root ID and empty `nodes`; it is not represented as an acquired document. `nodes` includes acquired rejected candidates for inspection. The propagation direction is always upstream `source` → downstream `target`; the seed normally appears as a target as ancestry expands.

| Edge state | Meaning |
| --- | --- |
| `validated` | Present in the actual traversal's accepted DAG. Its Ariadne score and mutations are copied unchanged. It is not a truth classification. |
...
| `candidate` | A fetched claim-only source match. It runs as an investigation root, but its source-to-submitted-text relationship is discovery only and has no Ariadne score. |

Render provenance links using **only** `status === "validated"`. Numeric values and `type` do not override state. In particular, a rejected proposal with a score is still rejected. A duplicate rejected proposal can coexist with a separately accepted relationship.
...
const graph = {
  nodes: result.nodes,
  links: result.edges.filter((x) => x.status === "validated"),
};
const displayState = {

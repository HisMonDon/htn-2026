...
Without a source field, `claim` itself becomes submitted source text. It has no publication date and no inferred outbound links. This supports the existing claim-only frontend call, but does not manufacture the temporal evidence needed to validate a chain. Submitted nodes have `source_kind: "submitted"`; their `url` is an internal `https://submitted.ariadne.invalid/...` identity needed by the existing document contract, not a fetched/publication URL. Do not offer it as an external source link. `root.url` is `null` for this case. Prefer a real `seed_url` for the demonstration.

`max_depth` defaults to 5, accepts 0–10, and counts accepted upstream hops from the seed. `max_provider_requests` defaults to 10 and accepts 1–10 per invocation. A limit can stop or pause a bounded run; `complete` does not claim the graph contains every possible ancestor. `include_ai_evidence` remains accepted for request compatibility but is deprecated: `true` produces an explicit warning and no AI-writing check. This endpoint runs bibliography proposals.
...
| `candidate` | Not validated or rejected yet. There is no Ariadne score. Reserved in the contract; the current traversal emits accepted/rejected edges and source-level `pending` records, not candidate edges for unknown parents. |
